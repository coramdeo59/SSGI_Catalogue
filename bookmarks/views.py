...
from usgsfeatures.models import SSGI  # Updated to use the correct class name
...
def bookmark_image(request, id):
    if request.method == "GET":
        ...
        image_instance = SSGI.objects.filter(id=id)[0]  # Updated to use SSGI
...
def bookmark_list(request):
    ...
    thumbnails = []
    foldercursor = SSGI.objects.filter().values()  # Updated to use SSGI
